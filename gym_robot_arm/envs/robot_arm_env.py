...
class RobotArmEnvV0(gym.Env):
    ...
    def final(self, theta):
        ...
        for i in range(1, len(P)):
            F_next = base.dot(P[i])
        self.x,self.y=int(F_next[0,3]), int(F_next[1,3])
    ...
    def step(self, action):
        ...
        self.final(self.theta)
        self.observation =np.array([self.x, self.y, self.target_pos[0], self.target_pos[1],self.theta[0],self.theta[1]])
        distance_error = np.sqrt((self.target_pos[0]-self.x)*(self.target_pos[0]-self.x)+(self.target_pos[1]-self.y)*(self.target_pos[1]-self.y) )
...
class RobotArmEnvV1(RobotArmEnvV0):
    ...
    def step(self, action):
        ...
        distance_error = euclidean(self.target_pos, tip_pos)#tip_pos vị trí đầu cánh tay

        # Sharp reward
        reward = -distance_error / 100
        epsilon=15
        done = distance_error < epsilon

        if (distance_error > -epsilon and distance_error < epsilon):
            done = True
        distance=self.distance_pos()
        observation = np.hstack((self.target_pos,self.x,self.y,self.theta))
        info = {
            'distance_error': distance_error,
            'target_position': self.target_pos,
